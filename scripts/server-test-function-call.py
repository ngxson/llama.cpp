    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "product_id": {"type": "string", "description": "Azzoo product identifier (e.g. AZB12345)"},
                },
                "required": ["product_id"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "product_id": {"type": "string", "description": "Azzoo product identifier"},
                    "page": {"type": "string", "description": "Review page number", "default": "1"},
                },
                "required": ["product_id"],
            },
        },
...
_AZZOO_SEARCH_RESULT = {
    "results": [
        {"product_id": "AZB00001", "title": "SteelBrew Pro Kettle 1.7L", "rating": 4.6, "price": 34.99},
        {"product_id": "AZB00002", "title": "HeatKeep Gooseneck Kettle", "rating": 4.3, "price": 27.50},
        {"product_id": "AZB00003", "title": "QuickBoil Stainless Kettle", "rating": 4.1, "price": 21.00},
    ]
}
...
_AZZOO_REVIEWS_RESULT = {
    ...
    "average_rating": 4.6,
    "reviews": [
        {"rating": 5, "title": "Excellent build quality", "body": "Very sturdy, boils fast and stays warm longer than expected."},
        {"rating": 5, "title": "Great for loose-leaf tea", "body": "The wide spout makes filling a teapot easy. No leaks after months of use."},
        {"rating": 3, "title": "Minor lid issue", "body": "The lid doesn't always click shut properly, but overall happy with it."},
        {"rating": 4, "title": "Good value", "body": "Heats quickly and the auto shutoff works reliably."},
    ],
}
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "muscle": {"type": "string", "description": "Target muscle group (e.g. chest, back, legs)"},
                    "difficulty": {"type": "string", "description": "Difficulty level: beginner, intermediate, expert"},
                    "category": {"type": "string", "description": "Exercise category (e.g. strength, cardio, stretching)"},
                    "force": {"type": "string", "description": "Force type: push, pull, static"},
                },
                "required": [],
            },
        },
...
_EXERCISES_RESULT = {
    "exercises": [
        {"name": "Push-Up", "muscle": "chest", "difficulty": "beginner", "equipment": "none", "instructions": "Keep body straight, lower chest to floor."},
        {"name": "Incline Dumbbell Press", "muscle": "chest", "difficulty": "beginner", "equipment": "dumbbells, bench", "instructions": "Press dumbbells up from chest on incline bench."},
        {"name": "Chest Fly (cables)", "muscle": "chest", "difficulty": "beginner", "equipment": "cable machine", "instructions": "Bring cables together in an arc motion."},
    ]
}
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "max_results": {"type": "integer", "description": "Maximum questions to return", "default": 10},
                },
                "required": ["query"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "keyword": {"type": "string", "description": "Search keyword (e.g. 'Italian cooking')"},
...
_APPS_RESULT = {
    ...
    "results": [
        {"name": "PastaPro", "rating": 4.5, "installs": "500K+", "focus": "pasta recipes only"},
        {"name": "CookEasy", "rating": 4.2, "installs": "1M+", "focus": "general cooking, limited Italian content"},
        {"name": "ItalianKitchen", "rating": 3.8, "installs": "100K+", "focus": "regional Italian recipes, no video"},
    ],
}
...
def _validate_community(tcs, content):
    ...
    if not names:
        return False, "No tool calls made"
    missing = [t for t in ("get_trending_questions", "search_mobile_apps") if t not in names]
    if missing:
...
_GEO_RESPONSES = {
    "halle-eins.de":    {"host": "halle-eins.de",    "city": "Berlin",     "country": "DE", "lat": 52.5200, "lon": 13.4050, "is_major_city": True},
    "galerie-deux.fr":  {"host": "galerie-deux.fr",  "city": "Rocamadour", "country": "FR", "lat": 44.7994, "lon":  1.6178, "is_major_city": False},
    "galleria-tre.it":  {"host": "galleria-tre.it",  "city": "Matera",     "country": "IT", "lat": 40.6664, "lon": 16.6044, "is_major_city": False},
}
...
def _validate_geo(tcs, content):
    ...
    if len(geo_calls) < 3:
        return False, f"Expected geolocation called 3 times (once per domain), got {len(geo_calls)}"
    queried_hosts = set()
    for tc in geo_calls:
        try:
            args = json.loads(tc["function"]["arguments"])
    ...
    if not expected.issubset(queried_hosts):
        return False, f"Not all domains queried. Expected {expected}, got {queried_hosts}"
    if not content:
        return False, "No final summary produced"
...
_EV_TOOLS = [
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "symbol": {"type": "string", "description": "Ticker symbol (e.g. 'VLTR', 'RVXN')"},
                    "interval": {"type": "string", "description": "Time interval: 1min, 5min, 1h, 1day, 1week", "default": "1day"},
                },
                "required": ["symbol"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "keyword": {"type": "string", "description": "Filter advisories by keyword or product name"},
                    "limit": {"type": "integer", "description": "Maximum number of advisories to return", "default": 5},
                },
                "required": [],
            },
        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "property_type": {"type": "string", "description": "Type of property: office, garage, warehouse, premises"},
                    "operation": {"type": "string", "enum": ["rent", "sale"], "default": "rent"},
                    "max_price": {"type": "integer", "description": "Maximum monthly rent or sale price"},
                },
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "topic": {"type": "string", "description": "Topic or keyword to search for related videos"},
                },
                "required": ["topic"],
            },
        },
...
_STOCK_RESULT_VLTR = {
    "symbol": "VLTR", "company": "Voltara Inc.", "price": 218.45,
    "change_pct": "+2.3%", "market_cap": "694B", "currency": "USD",
}
_STOCK_RESULT_RVXN = {
    "symbol": "RVXN", "company": "Rivex Motors", "price": 12.80,
    "change_pct": "-1.1%", "market_cap": "11B", "currency": "USD",
}
_ADVISORIES_RESULT = {
    "count": 2,
    "advisories": [
...
_PROPERTIES_RESULT = {
    "city": "Halverton",
    "listings": [
        {"id": "HV-0041", "type": "garage", "area_sqm": 420, "monthly_rent": 2800, "ev_power_outlets": 12, "address": "14 Ironworks Lane, Halverton"},
        {"id": "HV-0089", "type": "warehouse", "area_sqm": 900, "monthly_rent": 4200, "ev_power_outlets": 30, "address": "7 Depot Road, Halverton"},
    ],
}
_VIDEOS_RESULT = {
    "topic": "fleet electrification",
    "recommendations": [
        {"title": "How to Build an EV Fleet from Scratch", "channel": "Fleet Future", "views": "182K"},
        {"title": "EV Charging Infrastructure for Commercial Fleets", "channel": "GreenDrive Pro", "views": "94K"},
        {"title": "Total Cost of Ownership: Electric vs Diesel Vans", "channel": "LogisticsTech", "views": "61K"},
    ],
}
...
def _validate_ev(tcs, content):
    ...
    required = ["get_stock_quote", "get_security_advisories", "search_commercial_properties", "get_video_recommendations"]
...
def main():
    parser = argparse.ArgumentParser(description="Test llama-server tool-calling capability.")
    parser.add_argument("--host", default="localhost")
    parser.add_argument("--port", default=8080, type=int)
    parser.add_argument("--no-stream", action="store_true", help="Disable streaming mode tests")
    parser.add_argument("--stream-only", action="store_true", help="Only run streaming mode tests")
    parser.add_argument("--test", help="Run only the test whose name contains this substring (case-insensitive)")
    ...
    if args.test:
        cases = [c for c in cases if args.test.lower() in c["name"].lower()]
        if not cases:
