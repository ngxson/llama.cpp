...
int main(int argc, char ** argv) {
    ...
    if (ctx_server.params.enable_tool_calls) {
        ...
        if (tool_call_support) {
            LOG_WRN("Tool call is EXPERIMENTAL and maybe unstable. Use with your own risk\n");
        } else {
            LOG_ERR("Tool call is not supported for this model. Please remove --tool-call or use with a supported model\n");
            clean_up();
            t.join();
            return 1;
        }
    ...
    std::string template_example = llama_chat_format_example(ctx_server.model, params.chat_template);
    LOG_INF("%s: chat template: %s\nBuilt in template: %s\n", __func__, template_example.c_str(),
        params.chat_template.empty() ? "True" : "False"
    );
