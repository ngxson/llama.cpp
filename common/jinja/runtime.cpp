        ...
        for (size_t i = 0; i < expected_count; ++i) {
            if (i < input_count) {
                if (is_stmt<identifier>(this->args[i])) {
                    ...
                    JJ_DEBUG("  Binding parameter '%s' to argument of type %s", param_name.c_str(), args.get_kwarg_or_pos(param_name, i)->type().c_str());
                    macro_ctx.set_val(param_name, args.get_kwarg_or_pos(param_name, i));
