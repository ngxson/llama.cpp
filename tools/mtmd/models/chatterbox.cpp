...
// Chatterbox s3gen, stage 1: speech tokens -> mu (flow encoder output).
// Weights come from the source-named tensor map (model.cbx_tensors), the
// estimator and hift stages extend this file.

static ggml_tensor * cbx_t(const clip_model & model, const std::string & name) {
...
static ggml_tensor * cbx_linear(ggml_context * ctx0, ggml_tensor * w, ggml_tensor * b, ggml_tensor * x) {
...
static ggml_tensor * cbx_conv1d(ggml_context * ctx0, ggml_tensor * k, ggml_tensor * b, ggml_tensor * x,
...
static ggml_tensor * cbx_estimator(const clip_model & model, ggml_context * ctx0, ggml_tensor * x_noise, ggml_tensor * mu,
    ...
    // down
    ggml_tensor * skip;
    x = cbx_resnet(model, ctx0, x, temb, "est.down_blocks.0.0");
    for (int j = 0; model.cbx_tensors.count("est.down_blocks.0.1." + std::to_string(j) + ".norm1.weight"); j++) {
        x = cbx_tfm_block(model, ctx0, x, "est.down_blocks.0.1." + std::to_string(j));
    }
    skip = x;
    {
        ggml_tensor * k = cbx_t(model, "est.down_blocks.0.2.weight");
        x = cbx_conv1d(ctx0, k, cbx_t(model, "est.down_blocks.0.2.bias"), x, 1, (int) k->ne[0] - 1, 0);
    }

    // mid
    for (int i = 0; model.cbx_tensors.count("est.mid_blocks." + std::to_string(i) + ".0.block1.block.0.weight"); i++) {
        const std::string mp = "est.mid_blocks." + std::to_string(i);
    ...
    x = cbx_resnet(model, ctx0, x, temb, "est.up_blocks.0.0");
    for (int j = 0; model.cbx_tensors.count("est.up_blocks.0.1." + std::to_string(j) + ".norm1.weight"); j++) {
        x = cbx_tfm_block(model, ctx0, x, "est.up_blocks.0.1." + std::to_string(j));
    }
    {
        ggml_tensor * k = cbx_t(model, "est.up_blocks.0.2.weight");
        x = cbx_conv1d(ctx0, k, cbx_t(model, "est.up_blocks.0.2.bias"), x, 1, (int) k->ne[0] - 1, 0);
    }

    x = cbx_causal_block(model, ctx0, x, "est.final_block");
    x = cbx_conv1d(ctx0, cbx_t(model, "est.final_proj.weight"), cbx_t(model, "est.final_proj.bias"), x, 1, 0, 0); // [80, T]
    return x;
}
...
static ggml_cgraph * cbx_build_s3tok(const clip_model & model, ggml_context * ctx0, ggml_cgraph * gf, int T) {
    ...
    x = cbx_conv1d(ctx0, cbx_t(model, "s3tok.encoder.conv1.weight"), cbx_t(model, "s3tok.encoder.conv1.bias"), x, 2, 1, 1);
    x = ggml_gelu_erf(ctx0, x);
    x = cbx_conv1d(ctx0, cbx_t(model, "s3tok.encoder.conv2.weight"), cbx_t(model, "s3tok.encoder.conv2.bias"), x, 2, 1, 1);
    x = ggml_gelu_erf(ctx0, x); // [1280, T2]

    for (int li = 0; model.cbx_tensors.count("s3tok.encoder.blocks." + std::to_string(li) + ".attn_ln.weight"); li++) {
        const std::string p = "s3tok.encoder.blocks." + std::to_string(li) + ".attn";
        ...
        const std::string mp = "s3tok.encoder.blocks." + std::to_string(li) + ".mlp";
        res = x;
    ...
    x = cbx_linear(ctx0, cbx_t(model, "s3tok.quantizer._codebook.project_down.weight"),
                   cbx_t(model, "s3tok.quantizer._codebook.project_down.bias"), x); // [8, T2]
    x = ggml_tanh(ctx0, x);
...
ggml_cgraph * clip_graph_chatterbox::build() {
    ...
    // token embedding
    ggml_tensor * x = ggml_get_rows(ctx0, cbx_t(model, "flow.input_embedding.weight"), inp_tokens); // [512, T1]

    // embed: linear + layer norm, then the espnet xscale
    x = cbx_linear(ctx0, cbx_t(model, "fenc.embed.out.0.weight"), cbx_t(model, "fenc.embed.out.0.bias"), x);
    x = cbx_layer_norm(ctx0, cbx_t(model, "fenc.embed.out.1.weight"), cbx_t(model, "fenc.embed.out.1.bias"), x, 1e-5f);
    x = ggml_scale(ctx0, x, sqrtf(512.0f));

    // pre-lookahead: conv k=4 right-padded 3, leaky 0.01, conv k=3 left-padded 2, residual
    {
        ggml_tensor * res = x;
        ggml_tensor * cur = cbx_conv1d(ctx0, cbx_t(model, "fenc.pre_lookahead_layer.conv1.weight"),
                                       cbx_t(model, "fenc.pre_lookahead_layer.conv1.bias"), x, 1, 0, 3);
        cur = ggml_leaky_relu(ctx0, cur, 0.01f, false);
        cur = cbx_conv1d(ctx0, cbx_t(model, "fenc.pre_lookahead_layer.conv2.weight"),
                         cbx_t(model, "fenc.pre_lookahead_layer.conv2.bias"), cur, 1, 2, 0);
        x = ggml_add(ctx0, res, cur);
    }

    for (int i = 0; model.cbx_tensors.count("fenc.encoders." + std::to_string(i) + ".norm_mha.weight"); i++) {
        x = cbx_enc_layer(model, ctx0, x, pos1, "fenc.encoders." + std::to_string(i), T1);
    }
    ...
    // upsample x2: nearest repeat, left pad 4, conv k=5
    {
        ...
        z = ggml_scale(ctx0, z, 0.0f);
        xt = ggml_concat(ctx0, z, xt, 0);
        ggml_tensor * y = ggml_conv_1d(ctx0, cbx_t(model, "fenc.up_layer.conv.weight"), xt, 1, 0, 1);
        x = ggml_cont(ctx0, ggml_transpose(ctx0, y));                            // [512, T2]
        x = ggml_add(ctx0, x, cbx_t(model, "fenc.up_layer.conv.bias"));
    }

    // up embed: linear + layer norm + xscale
    x = cbx_linear(ctx0, cbx_t(model, "fenc.up_embed.out.0.weight"), cbx_t(model, "fenc.up_embed.out.0.bias"), x);
    x = cbx_layer_norm(ctx0, cbx_t(model, "fenc.up_embed.out.1.weight"), cbx_t(model, "fenc.up_embed.out.1.bias"), x, 1e-5f);
    x = ggml_scale(ctx0, x, sqrtf(512.0f));

    for (int i = 0; model.cbx_tensors.count("fenc.up_encoders." + std::to_string(i) + ".norm_mha.weight"); i++) {
        x = cbx_enc_layer(model, ctx0, x, pos2, "fenc.up_encoders." + std::to_string(i), T2);
    }

    x = cbx_layer_norm(ctx0, cbx_t(model, "fenc.after_norm.weight"), cbx_t(model, "fenc.after_norm.bias"), x, 1e-5f);
    ...
    auto time_mlp = [&](ggml_tensor * e) {
        e = cbx_linear(ctx0, cbx_t(model, "est.time_mlp.linear_1.weight"), cbx_t(model, "est.time_mlp.linear_1.bias"), e);
        e = ggml_silu(ctx0, e);
        e = cbx_linear(ctx0, cbx_t(model, "est.time_mlp.linear_2.weight"), cbx_t(model, "est.time_mlp.linear_2.bias"), e);
        return e;
    };
    auto span_emb = [&](int i) {
    ...
    auto step_temb = [&](int i) {
        ...
        return ggml_mul_mat(ctx0, cbx_t(model, "est.time_embed_mixer.weight"), e);                // [1024, 1]
    };
    ...
    {
        ggml_tensor * fx = mel;
        for (int i = 0; model.cbx_tensors.count("hift.f0_predictor.condnet." + std::to_string(i) + ".weight"); i += 2) {
            const std::string cp = "hift.f0_predictor.condnet." + std::to_string(i);
            fx = cbx_conv1d(ctx0, cbx_t(model, cp + ".weight"), cbx_t(model, cp + ".bias"), fx, 1, 1, 1);
            fx = ggml_elu(ctx0, fx);
        }
        fx = cbx_linear(ctx0, cbx_t(model, "hift.f0_predictor.classifier.weight"), cbx_t(model, "hift.f0_predictor.classifier.bias"), fx);
...
static ggml_tensor * cbx_conv1d_dil(ggml_context * ctx0, ggml_tensor * k, ggml_tensor * b, ggml_tensor * x, int pad, int dil) {
...
static ggml_cgraph * cbx_build_vocoder(const clip_model & model, ggml_context * ctx0, ggml_cgraph * gf, int n_mel, int n_stft) {
    ...
    ggml_tensor * x = cbx_conv1d_dil(ctx0, cbx_t(model, "hift.conv_pre.weight"), cbx_t(model, "hift.conv_pre.bias"), mel, 3, 1);

    for (int i = 0; model.cbx_tensors.count("hift.ups." + std::to_string(i) + ".weight"); i++) {
        const std::string is = std::to_string(i);
        ggml_tensor * uk = cbx_t(model, "hift.ups." + is + ".weight");
        ...
        x = ggml_add(ctx0, x, cbx_t(model, "hift.ups." + is + ".bias"));

        const bool is_last = !model.cbx_tensors.count("hift.ups." + std::to_string(i + 1) + ".weight");
        if (is_last) {
        ...
        ggml_tensor * sk = cbx_t(model, "hift.source_downs." + is + ".weight");
        ...
        ggml_tensor * si;
        {
            ...
            si = ggml_add(ctx0, si, cbx_t(model, "hift.source_downs." + is + ".bias"));
        }
        si = cbx_hift_resblock(model, ctx0, si, "hift.source_resblocks." + is);
        ...
        for (int j = 3 * i; j < 3 * (i + 1); j++) {
            ggml_tensor * r = cbx_hift_resblock(model, ctx0, x, "hift.resblocks." + std::to_string(j));
    ...
    x = cbx_conv1d_dil(ctx0, cbx_t(model, "hift.conv_post.weight"), cbx_t(model, "hift.conv_post.bias"), x, 3, 1);
