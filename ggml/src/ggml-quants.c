...
// =====================================================================
// Q5_K_HIFI_HYBRID: FP16 extreme outliers + INT8 moderate outliers
// Optimized for small models (≤1.7B) where extreme outliers need full precision
// =====================================================================

// Reference quantization: no imatrix, uses magnitude-based outlier detection
void quantize_row_q5_k_hifi_hybrid_ref(const float * GGML_RESTRICT x, block_q5_k_hifi_hybrid * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;

    for (int64_t ib = 0; ib < nb; ++ib) {
        const float * xb = x + ib * QK_K;
        block_q5_k_hifi_hybrid * block = &y[ib];

        // Find all outliers by magnitude (importance = |value|)
        float importance[QK_K];
        for (int i = 0; i < QK_K; ++i) {
            importance[i] = fabsf(xb[i]);
        }

        // Find top-7 outliers (4 extreme + 3 moderate)
        const int total_outliers = Q5_K_HIFI_HYBRID_MAX_EXTREME + Q5_K_HIFI_HYBRID_MAX_MODERATE;
        int outlier_indices[7];  // 4 + 3 = 7
        float outlier_mags[7];

        for (int k_idx = 0; k_idx < total_outliers; ++k_idx) {
            int max_idx = 0;
            float max_val = importance[0];
            for (int i = 1; i < QK_K; ++i) {
                if (importance[i] > max_val) {
            ...
            outlier_indices[k_idx] = max_idx;
            outlier_mags[k_idx] = max_val;
            importance[max_idx] = -1.0f;  // Mark as used
        }

        // Classify outliers: extreme (FP16) vs moderate (INT8)
        // Strategy: Top ~30% of outliers by magnitude get FP16
        // Use statistical threshold: extreme if > 0.7 * max_outlier_magnitude
        float max_outlier_mag = outlier_mags[0];  // Already sorted by magnitude
        float extreme_threshold = 0.7f * max_outlier_mag;

        int extreme_count = 0;
        int moderate_count = 0;
        int extreme_indices[Q5_K_HIFI_HYBRID_MAX_EXTREME];
        int moderate_indices[Q5_K_HIFI_HYBRID_MAX_MODERATE];

        for (int k_idx = 0; k_idx < total_outliers; ++k_idx) {
            if (outlier_mags[k_idx] >= extreme_threshold && extreme_count < Q5_K_HIFI_HYBRID_MAX_EXTREME) {
                extreme_indices[extreme_count++] = outlier_indices[k_idx];
            } else if (moderate_count < Q5_K_HIFI_HYBRID_MAX_MODERATE) {
                moderate_indices[moderate_count++] = outlier_indices[k_idx];
            }
        }

        block->extreme_count = (uint8_t)extreme_count;
        block->moderate_count = (uint8_t)moderate_count;

        // Zero all outliers and quantize Q5_K base
        float tmp[QK_K];
        memcpy(tmp, xb, QK_K * sizeof(float));
        for (int k_idx = 0; k_idx < extreme_count; ++k_idx) {
            tmp[extreme_indices[k_idx]] = 0.0f;
        }
        for (int k_idx = 0; k_idx < moderate_count; ++k_idx) {
            tmp[moderate_indices[k_idx]] = 0.0f;
        }
        quantize_row_q5_K_ref(tmp, (block_q5_K *)block, QK_K);

        // Store FP16 extreme outliers (full precision)
        for (int k_idx = 0; k_idx < extreme_count; ++k_idx) {
            const int idx = extreme_indices[k_idx];
            block->extreme_idx[k_idx] = (uint8_t)idx;
            block->extreme_vals[k_idx] = GGML_FP32_TO_FP16(xb[idx]);
        }
        for (int k_idx = extreme_count; k_idx < Q5_K_HIFI_HYBRID_MAX_EXTREME; ++k_idx) {
            block->extreme_idx[k_idx] = 0;
            block->extreme_vals[k_idx] = 0;
        }

        // Compute INT8 residuals for moderate outliers
        float dequant[QK_K];
        dequantize_row_q5_K((const block_q5_K *)block, dequant, QK_K);

        float max_residual = 0.0f;
        float residuals[Q5_K_HIFI_HYBRID_MAX_MODERATE];
        for (int k_idx = 0; k_idx < moderate_count; ++k_idx) {
            const int idx = moderate_indices[k_idx];
            residuals[k_idx] = xb[idx] - dequant[idx];
            if (fabsf(residuals[k_idx]) > max_residual) {
                max_residual = fabsf(residuals[k_idx]);
            }
        ...
        if (max_residual == 0.0f) max_residual = 1e-8f;
        block->moderate_scale = max_residual;

        for (int k_idx = 0; k_idx < moderate_count; ++k_idx) {
            block->moderate_idx[k_idx] = (uint8_t)moderate_indices[k_idx];
            float norm_res = residuals[k_idx] / max_residual;
            block->moderate_residuals[k_idx] = (int8_t)roundf(norm_res * 127.0f);
        }
        for (int k_idx = moderate_count; k_idx < Q5_K_HIFI_HYBRID_MAX_MODERATE; ++k_idx) {
            block->moderate_idx[k_idx] = 0;
            block->moderate_residuals[k_idx] = 0;
        }
    }
}

// imatrix-aware quantization: uses weighted importance for outlier selection
static void quantize_row_q5_k_hifi_hybrid_impl(const float * GGML_RESTRICT x, block_q5_k_hifi_hybrid * GGML_RESTRICT y, int64_t k, const float * GGML_RESTRICT quant_weights) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
    ...
    for (int64_t ib = 0; ib < nb; ++ib) {
        ...
        // Compute imatrix-weighted importance
        float importance[QK_K];
        for (int i = 0; i < QK_K; ++i) {
            float weight = qw ? qw[i] : 1.0f;
            importance[i] = fabsf(xb[i]) * weight;
        ...
        // Find top-7 outliers by weighted importance
        const int total_outliers = Q5_K_HIFI_HYBRID_MAX_EXTREME + Q5_K_HIFI_HYBRID_MAX_MODERATE;
        int outlier_indices[7];
        float outlier_mags[7];  // Store raw magnitudes for threshold calculation
        ...
        for (int k_idx = 0; k_idx < total_outliers; ++k_idx) {
            ...
            outlier_indices[k_idx] = max_idx;
            outlier_mags[k_idx] = fabsf(xb[max_idx]);  // Raw magnitude, not weighted
            importance[max_idx] = -1.0f;
        }

        // Classify by raw magnitude: extreme if > 0.7 * max_raw_magnitude
        float max_raw_mag = outlier_mags[0];
        for (int k_idx = 1; k_idx < total_outliers; ++k_idx) {
            if (outlier_mags[k_idx] > max_raw_mag) max_raw_mag = outlier_mags[k_idx];
        }
        float extreme_threshold = 0.7f * max_raw_mag;
        ...
        // Store FP16 extreme outliers (full precision - critical for small models)
        for (int k_idx = 0; k_idx < extreme_count; ++k_idx) {
            const int idx = extreme_indices[k_idx];
            block->extreme_idx[k_idx] = (uint8_t)idx;
            block->extreme_vals[k_idx] = GGML_FP32_TO_FP16(xb[idx]);
...
// Dequantization: Q5_K base + FP16 extreme + INT8 moderate corrections
void dequantize_row_q5_k_hifi_hybrid(const block_q5_k_hifi_hybrid * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;

    for (int64_t ib = 0; ib < nb; ++ib) {
        const block_q5_k_hifi_hybrid * block = &x[ib];
        float * yb = y + ib * QK_K;

        // Dequantize Q5_K base
        dequantize_row_q5_K((const block_q5_K *)block, yb, QK_K);

        // Apply FP16 extreme outliers (full replacement - maximum precision)
        const int extreme_count = block->extreme_count;
        for (int k_idx = 0; k_idx < extreme_count; ++k_idx) {
            const int idx = block->extreme_idx[k_idx];
            yb[idx] = GGML_FP16_TO_FP32(block->extreme_vals[k_idx]);
        }

        // Add INT8 moderate residual corrections
        const int moderate_count = block->moderate_count;
        const float scale = block->moderate_scale;
        for (int k_idx = 0; k_idx < moderate_count; ++k_idx) {
            const int idx = block->moderate_idx[k_idx];
            const float residual = scale * (block->moderate_residuals[k_idx] / 127.0f);
            yb[idx] += residual;
        }
    }
}
...
// Public quantization function with imatrix support
size_t quantize_q5_k_hifi_hybrid(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    size_t row_size = ggml_row_size(GGML_TYPE_Q5_K_HIFI_HYBRID, n_per_row);

    if (!quant_weights) {
        char * qrow = (char *)dst;
        for (int64_t row = 0; row < nrow; ++row) {
            quantize_row_q5_k_hifi_hybrid_ref(src, (block_q5_k_hifi_hybrid*)qrow, n_per_row);
            src += n_per_row;
            qrow += row_size;
        }
    } else {
        char * qrow = (char *)dst;
        for (int64_t row = 0; row < nrow; ++row) {
            quantize_row_q5_k_hifi_hybrid_impl(src, (block_q5_k_hifi_hybrid*)qrow, n_per_row, quant_weights);
            src += n_per_row;
            qrow += row_size;
        }
    }
...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_Q5_K_HIFI_HYBRID:
            {
                VALIDATE_ROW_DATA_D_F16_IMPL(block_q5_k_hifi_hybrid, data, nb);
            } break;
