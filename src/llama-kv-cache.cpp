    ...
    for (uint32_t s = 0; s < sinfo.n_stream(); ++s) {
        for (uint32_t ii = 0; ii < sinfo.size(); ++ii) {
            ...
            if (ubatch.is_pos_2d() || ubatch.token) {
                ...
                // remember the token id so that n-gram embeddings can look back at it later
                if (ubatch.token) {
...
bool llama_kv_cache::has_cell_ext() const {
    // M-RoPE stores the 2D position, n-gram embeddings store the token id
    return hparams.n_pos_per_embd() > 1 || hparams.n_ngram() > 0;
}
...
void llama_kv_cache::get_prev_tokens(const llama_ubatch & ubatch, uint32_t n, std::vector<llama_token> & res) const {
    ...
    res.clear();
    res.resize(n_tokens*n, -1);

    if (n == 0) {
        return;
    }

    // note: apply_ubatch() has already stored the current ubatch, so the window below can also
    //       contain tokens of this very ubatch - which is exactly what we want
    ...
    for (uint32_t i = 0; i < n_tokens; ++i) {
        ...
        //       the n-gram architectures reject such batches, see llama_model_longcat_ngram
        const llama_seq_id seq_id = ubatch.seq_id[i][0];
...
bool llama_kv_cache::state_read_meta(llama_io_read_i & io, uint32_t strm, uint32_t cell_count, slot_info & sinfo, llama_seq_id dest_seq_id) {
    ...
    if (dest_seq_id != -1) {
        ...
        // note: apply_ubatch() rebuilds llama_kv_cell_ext from the ubatch, so only the parts that
        //       round-trip through it are restored (ext.tok, and the 2D position for M-RoPE)
